...
class GUI:
    ...
    def get_device_channels(self):
            ...
            本函数旨在确定设备的最大输入和输出通道数，并返回其中较小的一个，
            但最多不超过2个通道。这是为了确保设备在立体声模式下能够正常工作，
            因为立体声模式最多需要2个通道（左、右声道）。

            Returns:
                int: 设备的输入和输出通道数中较小的一个，但不超过2个通道。
            """
            # 获取默认输入设备的最大输入通道数
