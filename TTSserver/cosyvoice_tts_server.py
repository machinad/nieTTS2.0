import logging
logger = logging.getLogger(__name__)
import dashscope
from dashscope.audio.tts_v2 import SpeechSynthesizer as SpeechSynthesizerV2
from dashscope.audio.tts import SpeechSynthesizer as SpeechSynthesizerV1
import uuid
import config.default as config

class CosyVoiceTTS:
    def __init__(self,voice,prompt):
        self.config_manager = config.ConfigManager()
        self.voice = voice
        self.prompt = prompt
        self.path = self.config_manager.save_path
    async def use_cosyvoice_tts(self):
        model = "cosyvoice-v1"
        voice = self.voice
        Key = self.config_manager.get("ali_api_key")
        save_path = self.path / f"{uuid.uuid4()}.mp3"
        CosyVoiceTTS_tts_task = {
            "voice": self.voice,
            "prompt": self.prompt,
            "path": save_path,
            "status": "running"
        }
        try:
            if Key == "":
                logger.error("阿里百炼API密钥为空")
                CosyVoiceTTS_tts_task["status"] = "failed"
                return CosyVoiceTTS_tts_task
            dashscope.api_key = Key
            synthesizer = SpeechSynthesizerV2(model=model, voice=voice)
            audio = synthesizer.call(self.prompt)
        except Exception as e:
            logger.error(f"阿里百炼转换失败: {e}")
            CosyVoiceTTS_tts_task["status"] = "failed"
            return CosyVoiceTTS_tts_task
        with open(save_path, 'wb') as f:
            f.write(audio)
        CosyVoiceTTS_tts_task["status"] = "success"
        return CosyVoiceTTS_tts_task