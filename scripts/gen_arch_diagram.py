...
def arrow_right(parent, x, y, dest_x, color="#58a6ff"):
    ...
    ET.SubElement(
        parent, "polygon", attrib={"points": f"{dest_x},{y} {dest_x - 8},{y - 5} {dest_x - 8},{y + 5}", "fill": color}
    )
...
def arrow_down(parent, x, y, dest_y, color="#58a6ff"):
    ...
    ET.SubElement(
        parent, "polygon", attrib={"points": f"{x},{dest_y} {x - 5},{dest_y - 8} {x + 5},{dest_y - 8}", "fill": color}
    )
...
# Title
svg_text(g, W / 2, 46, "nieTTS 2.0  —  Architecture Overview", size=20, fill=C["accent"], bold=True, anchor="middle")
svg_text(
    g,
    W / 2,
    66,
    "Request Queue (Serial)  →  Parallel Processing  →  Play Queue (FIFO)",
    size=11,
    fill=C["muted"],
    anchor="middle",
)
...
subbox(g, x0 + 12, y0 + 50, 216, 52, fill=C["dark_green"], stroke=C["green"], title="TTS Control Panel")
...
subbox(g, x0 + 12, y0 + 112, 100, 52, fill=C["dark_green"], stroke=C["green"], title="WebSocket (↑)")
svg_text(g, x0 + 22, y0 + 134, "Audio Stream", size=9, fill=C["green"])
svg_text(g, x0 + 22, y0 + 148, "Int16 44.1kHz mono", size=8, fill=C["muted"])

subbox(g, x0 + 128, y0 + 112, 100, 52, fill=C["dark_green"], stroke=C["green"], title="WebSocket (↓)")
...
svgs = ["TTS Provider · Voice", "Translate · OSC · Playback", "STT · VAD models (P2)", "Device · Language · API keys"]
...
# ======================== CORE PIPELINE ========================
x2, y2 = 680, 90
box(g, x2, y2, 670, 680, color=C["orange"], title="RequestPipeline", subtitle="engines/pipeline.py — 核心调度器")
...
for i in range(5):
    ...
    svg_text(g, qx + 21, rq_y + 46, f"R{i + 1}", size=8, fill="#0d1117", bold=True, anchor="middle")
...
# --- _process() ---
pr_y = rq_y + 90
subbox(g, x2 + 16, pr_y, 638, 210, fill=C["queue_bg"], stroke=C["green"], title="_process() — asyncio.gather 并行执行")

bx, by = x2 + 36, pr_y + 34

# Left branch: TTS Original
subbox(g, bx, by, 290, 132, fill=C["dark_green"], stroke=C["green"], title="TTS(原文)")
...
bx2 = bx + 310
subbox(g, bx2, by, 308, 132, fill=C["dark_purple"], stroke=C["purple"], title="[if translate] 翻译 + OSC + 译文TTS")
...
svg_text(g, bx2 + 16, by + 64, '2. OSCService.send(原文 + "\\n" + 译文)', size=9, fill=C["purple"])
...
# --- Push order ---
push_y = pr_y + 226
subbox(g, x2 + 16, push_y, 638, 54, fill=C["queue_bg"], stroke=C["accent"], title="按序 push 到 PlayQueue")
...
# --- PlayQueue ---
pq_y = push_y + 74
subbox(g, x2 + 16, pq_y, 638, 50, fill=C["queue_bg"], stroke=C["red"], title="PlayQueue  (asyncio.Queue, FIFO, 纯播放)")
...
for i in range(6):
    ...
    svg_text(g, qx + 21, pq_y + 38, ".mp3", size=7, fill="#0d1117", bold=True, anchor="middle")
...
svg_text(
    ...
    x2 + 26,
    pw_y + 30,
    "原生 WAV / MP3 / OGG / FLAC · 无需 ffmpeg · asyncio.to_thread 避免阻塞",
    size=9,
    fill=C["muted"],
)
...
# --- Phase 2 preview ---
ph2_y = pw_y + 68
subbox(g, x2 + 16, ph2_y, 638, 44, fill=C["dark_purple"], stroke=C["red"], title="Phase 2: Voice Input Pipeline")
...
ET.SubElement(g, "polygon", points=f"{x1 - 5},{y0 + 112} {x1 - 13},{y0 + 107} {x1 - 13},{y0 + 117}", fill=C["green"])
svg_text(g, x0 + 245, y0 + 128, "audio(↑)", size=8, fill=C["green"])

svg_line(g, x1 - 5, y0 + 158, x0 + 240, y0 + 158, stroke=C["purple"], sw=1.5)
ET.SubElement(
    g, "polygon", points=f"{x0 + 240},{y0 + 158} {x0 + 248},{y0 + 153} {x0 + 248},{y0 + 163}", fill=C["purple"]
)
...
ET.SubElement(
    g, "polygon", points=f"{x0 + 240},{y0 + 250} {x0 + 248},{y0 + 245} {x0 + 248},{y0 + 255}", fill=C["purple"]
)
